...
public class Validation implements Validated, Cloneable, Constrained, ConstrainedPropertyListener {
    ...
    public ValidationGroup addGroup(String name) {
        ...
        if (null == validationGroups_) {
            validationGroups_ = new LinkedHashMap<>();
        }

        var group = new ValidationGroup(name, this);
    ...
    public void focusGroup(String name) {
        ...
        var group = validationGroups_.get(name);
        if (null == group) {
            return;
        }

        Set<ValidationError> retained_errors = new LinkedHashSet<>();
        var retained_subjects = group.getSubjects();
        for (var error : validationErrors_) {
    ...
    public void resetGroup(String name) {
        ...
        Set<ValidationError> retained_errors = new LinkedHashSet<>();
        var group_subjects = group.getSubjects();
        for (var error : validationErrors_) {
    ...
    public void addRule(ValidationRule rule) {
        ...
        if (null == validationRules_) {
            validationRules_ = new ArrayList<>();
        }
        if (null == validatedSubjects_) {
            validatedSubjects_ = new ArrayList<>();
        }
    ...
    public List<PropertyValidationRule> generateConstrainedPropertyRules(ConstrainedProperty constrainedProperty) {
        List<PropertyValidationRule> rules = new ArrayList<>();
        ...
        if (constrainedProperty.hasMimeType()) {
            Class<?> mime_type_validation;
            try {
                mime_type_validation = Class.forName("rife.cmf.MimeTypeValidation");
                var get_validation_rule = mime_type_validation.getDeclaredMethod("getValidationRule", MimeType.class, ConstrainedProperty.class);
                PropertyValidationRule rule = (PropertyValidationRule) get_validation_rule.invoke(null, constrainedProperty.getMimeType(), constrainedProperty);
                if (rule != null) {
                    rules.add(rule);
                }
    ...
    public List<PropertyValidationRule> addConstrainedPropertyRules(ConstrainedProperty constrainedProperty) {
        ...
        if (null == constrainedProperties_) {
            constrainedProperties_ = new LinkedHashMap<>();
        }

        // store the constrained property and obtain the old one if it exists
        var old_constrained_property = constrainedProperties_.put(constrainedProperty.getPropertyName(), constrainedProperty);
        if (old_constrained_property != null &&
            ...
            var rules_to_remove = new ArrayList<ValidationRule>();
            for (var rule : validationRules_) {
            ...
            // merge constraints
            Map<String, Object> merged_constraints = new HashMap<>(old_constrained_property.getConstraints());
        ...
        var rules = generateConstrainedPropertyRules(constrainedProperty);
        for (var rule : rules) {
    ...
    public void addConstraint(ConstrainedBean constrainedBean) {
        ...
        if (constrainedBean_ != null) {
            var merged_constraints = constrainedBean_.getConstraints();
    ...
    public void addValidationError(ValidationError newError) {
        ...
        if (null == validationErrors_) {
            validationErrors_ = new LinkedHashSet<>();
        }
    ...
    public void limitSubjectErrors(String subject) {
        ...
        if (null == errorLimitedSubjects_) {
            errorLimitedSubjects_ = new ArrayList<>();
        }
    ...
    public void makeSubjectValid(String subject) {
        ...
        var errors_to_remove = new ArrayList<ValidationError>();
        ...
        for (var error_to_remove : errors_to_remove) {
            validationErrors_.remove(error_to_remove);
        }
    }
    ...
    throws CloneNotSupportedException {
        ...
        try {
            ...
            if (validationRules_ != null) {
                ...
                for (var rule : new_validation.validationRules_) {
            ...
            if (validationErrors_ != null) {
                new_validation.validationErrors_ = new LinkedHashSet<>(validationErrors_);
            }

            if (constrainedProperties_ != null) {
                new_validation.constrainedProperties_ = new LinkedHashMap<>();
                for (var entry_property : constrainedProperties_.entrySet()) {
            ...
            if (activePropertyConstraints_ != null) {
                new_validation.activePropertyConstraints_ = new HashSet<>(activePropertyConstraints_);
            }

            if (errorLimitedSubjects_ != null) {
                new_validation.errorLimitedSubjects_ = new ArrayList<>(errorLimitedSubjects_);
            }

            if (validationGroups_ != null) {
                new_validation.validationGroups_ = new LinkedHashMap<>();
                ValidationGroup new_group;
                for (var group : validationGroups_.values()) {
